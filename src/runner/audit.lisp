     ...
     (let ((ht (make-hash-table :test 'equal)))
       (loop for (k v) on val by #'cddr
             do (setf (gethash (key-to-string k) ht)
                      (coerce-to-json-compatible v)))
       ht))
    ((librecode-runner.protocol:failure-descriptor-p val)
     (let ((ht (make-hash-table :test 'equal)))
       (setf (gethash "type" ht) (coerce-to-json-compatible (librecode-runner.protocol:failure-descriptor-type val)))
       (setf (gethash "message" ht) (librecode-runner.protocol:failure-descriptor-message val))
       (setf (gethash "initargs" ht) (coerce-to-json-compatible (librecode-runner.protocol:failure-descriptor-initargs val)))
       ht))
    ((listp val)
